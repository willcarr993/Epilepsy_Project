...
while 1
    if ptb3
        subject = str2double(Ask(screenparms.window,'(ptb3) Enter Subject Number (followed by Enter or Return):',...
            screenparms.white,screenparms.black, 'GetChar', RectLeft, RectTop, 10));
    else
        subject = GetEchoNumber(screenparms.window, ...
            'Enter Subject Number (followed by Enter or Return):',...
            100,100,screenparms.white,screenparms.black);
    end
